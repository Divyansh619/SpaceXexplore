...
export function RocketDetail() {
  ...
  // Data enrichment: fetch all launches for this rocket
  const { 
    data: launches, 
    ...
    {
      enabled: !!rocket, // Only fetch launches once we have the rocket data
    }
  );
  ...
  if (isErrorRocket || !rocket) {
    return (
      <Stack align="center" mt={50}>
        <Text color="red" size="lg">Error loading rocket details</Text>
  ...
  return (
    <Stack spacing="lg">
      ...
      <Grid>
        <Grid.Col span={5}>
          <Image
            src={rocket.flickr_images[0] || 'https://via.placeholder.com/500x500?text=No+Image'}
            alt={rocket.name}
            radius="md"
          />
        ...
        <Grid.Col span={7}>
          ...
          <SimpleGrid cols={2} spacing="md">
            ...
            <Card withBorder padding="sm">
              ...
              <Text ml={26}>{rocket.height.meters}m / {rocket.height.feet}ft</Text>
            </Card>
            ...
            <Card withBorder padding="sm">
              ...
              <Text ml={26}>{rocket.engines.number}x {rocket.engines.type}</Text>
            </Card>
          </SimpleGrid>
      ...
      <Grid>
        <Grid.Col span={6}>
          <Paper withBorder p="md">
            <Title order={4} mb="sm">Dimensions</Title>
            <List>
              <List.Item>Height: {rocket.height.meters}m / {rocket.height.feet}ft</List.Item>
              <List.Item>Diameter: {rocket.diameter.meters}m / {rocket.diameter.feet}ft</List.Item>
              <List.Item>Mass: {rocket.mass.kg.toLocaleString()}kg / {rocket.mass.lb.toLocaleString()}lb</List.Item>
        ...
        <Grid.Col span={6}>
          <Paper withBorder p="md">
            <Title order={4} mb="sm">Engine Details</Title>
            <List>
              ...
              <List.Item>Thrust to Weight: {rocket.engines.thrust_to_weight}</List.Item>
            </List>
          </Paper>
        </Grid.Col>
      </Grid>
      ...
      {isLoadingLaunches ? (
        ...
        <Text color="red" align="center">Error loading launches</Text>
      ) : launches && launches.length > 0 ? (
        <>
          <Text mb="md">This rocket has been used in {launches.length} launches:</Text>
        ...
        <Text align="center" italic>No launches found for this rocket</Text>
      )}
    </Stack>
  );
}